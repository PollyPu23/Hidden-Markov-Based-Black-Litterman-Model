...
def get_ind_30_ret():
    ...
    """    
    ind = pd.read_csv(f"../Data/ind_30_vw_monthly_ret.csv", header=0, index_col=0, na_values=-99.99)/100
...
def get_risk_free_rate():
    """
    Load and format the Ken French Monthly Risk Free Rate
    """
    rf = pd.read_csv('../Data/RF.csv', header=0, index_col=0, na_values=-99.99)/100
    rf.index = pd.to_datetime(rf.index, format="%Y%m").to_period('M')
    rf.interpolate(method='linear', limit_direction='both')
    return rf
...
def sharpe_ratio(r, riskfree_rate=0.04, periods_per_year=12):
    """
